...
import { Printer, Share, ShoppingCart, CheckCircle, Truck, Package , Pencil, BadgeCheck } from 'lucide-react';
...
const trackingSteps = [
  { status: 'Order Placed', date: 'Nov 03, 2022', icon: <ShoppingCart size={20} className="text-blue-400" /> },
  { status: 'Picked', date: 'Nov 03, 15:10', icon: <CheckCircle size={20} className="text-green-500" /> },
  { status: 'Shipping', date: 'Nov 03, 15:10', icon: <Truck size={20} className="text-red-500" /> },
  { status: 'Out For Delivery', date: 'Nov 03, 15:10 (expected)', icon: <Package size={20} className="text-purple-500" /> },
  { status: 'Delivered', date: 'Nov 03, 18:42', icon: <Package size={20} className="text-purple-500" /> },
];
const orderDetails = {
  orderNo: "VTK-1023",
...
const AdminOrderDetails = () => { 
  ...
  <div className="grid grid-cols-6 grid-rows-5 gap-2">
    ...
    <div className="bg-dark text-white p-6 rounded-lg">
            ...
            <tr key={item.id} className="border-b border-gray-700">
              <td className="p-3 flex items-center space-x-3">
                ...
                <div>
                  ...
                  {item.badge && <span className="text-xs bg-blue-500 px-2 py-1 rounded text-white">{item.badge}</span>}
                </div>
              </td>
      ...
      <div className="mt-6 border-t border-gray-700 pt-4">
        ...
        <div className="flex justify-between mb-2">
          ...
          <span className="bg-green-500 px-2 py-1 rounded text-white">{orderDetails.appliedCoupon}</span>
        </div>
        <div className="flex justify-between mb-2">
    ...
    <div className="col-span-2 row-span-5 col-start-3 p-4">
