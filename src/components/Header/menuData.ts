...
const TARGET_MAIN_CATEGORIES = ["VW-T5", "VW-T6.1", "VW-T7", "Universal Camper Parts"];
// ✅ Generate dynamic submenu paths
menuData.forEach((main) => {
  if (TARGET_MAIN_CATEGORIES.includes(main.title) && main.submenu) {
    main.submenu = main.submenu.map((sub) => ({
      ...sub,
      path:"/shop",
      apiUrl: `/shop?mainCategory=${encodeURIComponent(main.title)}&subCategory1=${encodeURIComponent(sub.title)}`,
    }));
  }
});